...
class FittsTestUI extends UIClass {
    ...
    newTrial() {
        ...
        if (this.trialCount > this.MAX_TRIALS) {
            ...
            // === YOUR CODE HERE ===
            //Set the reticle's center coordinates to random coordinates
...
//=====================================================================
// Sub-classes implementing the types of i nteractive objects we use
//=====================================================================
...
class Reticle extends Target {
    ...
    override draw(ctx : CanvasRenderingContext2D) : void {
        ...
        if(this.parentUI.currentState=='begin_trial'){
            ...
            //The lines starting points will be determined by adding the radius to the center points
            //For the vertical crosshair line, the Y coordinate of the starting point will be centerY + RETICLE_DIAM/2 and centerY - RETICLE_DIAM/2
            //And the X coordinate remains the same
            ctx.beginPath();
            ctx.moveTo(this.centerX, (this.centerY + (Reticle.RETICLE_DIAM/2)));
            ctx.lineTo(this.centerX, (this.centerY - (Reticle.RETICLE_DIAM/2)));
            ctx.stroke();

            //For the vertical crosshair line, the X coordinate of the starting point will be centerX + RETICLE_DIAM/2 and centerX - RETICLE_DIAM/2
            //And the X coordinate remains the same
            ctx.beginPath();
...
class BackgroundDisplay extends ScreenObject{
    ...
    override handleClickAt(ptX : number, ptY : number) : boolean {
        ...
        // === YOUR CODE HERE ===
        // console.log("clicked in background display");
        // console.log(this.parentUI.currentState);
